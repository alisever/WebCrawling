        ...
        if '/yazarlar/' in response.request.url:
            yield {
                ...
                'Tarih': (response.css('div.author-column__date > time::text').
                          ...
                          replace('\t', '').partition(',')[0]),
                'Detay': (''.join(response.css('div.author-column__content ::text').getall()).replace("\r", "").
                          replace("\n", "").replace("\t", "").
                          replace("\xa0", " ").strip()),
            ...
            summary = (''.join(response.css('h2.news-detail__info__spot ::text').getall()).
                       replace("\r", "").replace("\n", "").replace("\t", "").replace("\xa0", " ").strip())
            text = (''.join(response.css('div.news-detail__body__content.clearfix ::text').getall()).
                    replace("\r", "").replace("\n", "").replace("\t", "").replace("\xa0", " ").strip())
            if text == '':
                text = (''.join(response.css('div.news-detail--photo__text-content ::text').getall()).
                        replace("\r", "").replace("\n", "").replace("\t", "").replace("\xa0", " ").strip())
            yield {
                'Url': response.request.url,
